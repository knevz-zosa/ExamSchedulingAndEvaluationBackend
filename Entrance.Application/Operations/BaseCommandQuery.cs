...
namespace Entrance.Application.Operations;
//CREATE
...
//DETAILS
public abstract class BaseDetailsQuery<TResponse> : IRequest<ResponseWrapper<TResponse>>
{
    public int Id { get; set; }

    protected BaseDetailsQuery(int id)
    {
        Id = id;
    }
}
public abstract class BaseDetailsQueryHandler<TQuery, TResponse> : IRequestHandler<TQuery, ResponseWrapper<TResponse>>
    where TQuery : BaseDetailsQuery<TResponse>
{
    protected readonly IUnitOfWork<int> _unitOfWork;
    protected readonly IMapper _mapper;

    public BaseDetailsQueryHandler(IUnitOfWork<int> unitOfWork, IMapper mapper)
    {
        _unitOfWork = unitOfWork;
        _mapper = mapper;
    }
