...
public class UndergraduateRegistrationShould : TestBaseIntegration
{
    ...
    public async Task PerfomUndergraduateRegistrationMethods()
    {
        ...
        if (sched != null)
        {
            var existingLrn = new ApplicantRequest
            {
                ScheduleId = sched.Id,
                CourseId = sched.Campus.Courses.FirstOrDefault().Id,
            ...
            var exceptionCreate = await Connect.Undergraduate.Application(existingLrn);

            // Assert: Check the exception message
            Assert.False(exceptionCreate.IsSuccessful);
            Assert.Equal("LRN already exists.", exceptionCreate.Messages.FirstOrDefault());
            ...
            var existingapplicant = new PersonalInformationRequest
            {
                ...
                DateofBirth = applicant.PersonalInformation.DateofBirth,
                HouseNo = "163",
            ...
            // Act & Assert: Expect a ResponseException to be thrown for the duplicate
            exceptionCreate = await Connect.Undergraduate.PersonalInformation(existingapplicant);
            ...
            //update Registered to True
            var updateRegistered = new ApplicantUpdateRegistered
            {
                Id = applModel.Data.Id,
                Registered = true
            };

            var updateRegisteredResult = await Connect.Applicant.UpdateRegistered(updateRegistered);
            Assert.True(updateRegisteredResult.IsSuccessful);
            ...
            var Models = await Connect.Applicant.List(listQuery, "All");
            ...
            //Assert.True(Models.Data.List.Any(x => x.Id == applModel.Data.Id),
            //        $"Item with Id {applModel.Data.Id} not found in the list.");
        }
    }
}
