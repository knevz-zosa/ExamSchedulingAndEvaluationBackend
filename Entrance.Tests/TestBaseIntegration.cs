using Entrance.Api.Controllers;
...
public class TestBaseIntegration : IDisposable
{
    ...
    public TestBaseIntegration()
    {
        ...
        var examResultService = new ExamResultService(Http);
        var interviewResultService = new InterviewResultService(Http);
    ...
    private async Task SeedApplicantsForTest(IServiceProvider serviceProvider)
    {
        ...
        if (response == true)
        {
            var councelorRequest = new CounselorConsultation
            {
                ApplicantId = applicant.Id,
    ...
    protected async Task<ResponseWrapper<PagedList<ScheduleListResponse>>> SchedulesDefault()
    {
        ...
        var listQuery = new DataGridQuery
        {
            Page = 0,
            PageSize = 100,
            SortField = nameof(ScheduleListResponse.ScheduleDate),
    ...
    protected async Task<ResponseWrapper<PagedList<ApplicantListResponse>>> ApplicantsDefault()
    {
        var user = await LoginDefault();
        var listQuery = new DataGridQuery
        {
            Page = 0,
            PageSize = 100,
            SortField = nameof(ApplicantListResponse.FullName),
            SortDir = DataGridQuerySortDirection.Ascending
        };

        var applicantModels = await Connect.Applicant.List(listQuery, user.Data.Access);
